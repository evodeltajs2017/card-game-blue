class Validator {
	...
	initialize(){
		...
		this.validateDamage();
		this.validateHealth();
		this.isNameUnique(this.objectToValidate.name, (isUnique) => {
			if (isUnique == false)
				this.responseArr.push("The card already exists");
			this.callback(this.responseArr);
		});
	}

	validateName(){
		console.log("I'm also here");
		console.log("something here",this.objectToValidate);
